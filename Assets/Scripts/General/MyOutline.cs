...
namespace General
{
  ...
  public class MyOutline : MonoBehaviour {
    static readonly HashSet<Mesh> registeredMeshes = new();
    static readonly int color = Shader.PropertyToID("_OutlineColor");
    ...
    [SerializeField, HideInInspector] List<Mesh> bakeKeys = new();

    [SerializeField, HideInInspector] List<ListVector3> bakeValues = new();
    ...
    void LoadSmoothNormals() {
      ...
      foreach (var meshFilter in GetComponentsInChildren<MeshFilter>()) {
        ...
        if (r != null) {
          CombineSubmeshes(meshFilter.sharedMesh, r.sharedMaterials);
        }
      }
